...
def train(training_data_loader, optimizer, model, epoch, num_epochs):
	...
	#-------------------------l2 loss-------------------------
        ...
        PPI_loss = PPI_loss + wavelet_loss_PPI*alpha

        loss_demosaic = l2_loss + wavelet_loss_demosaic*alpha + mosaic_loss*beta
        loss_x4 = PPI_loss*10000 + loss_demosaic
...
def save_checkpoint(model, epoch):
    model_folder = "checkpoint/main/"
    model_out_path = model_folder + "main_model_epoch_{}.pth".format(epoch)
...
def save_opt(opt):
    statistics_folder = "checkpoint/main/"
